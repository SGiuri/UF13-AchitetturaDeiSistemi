...
@app.route("/posts/<string:author_username>")
def users_posts(author_username):
    # recupera tutti i post dal database
    user = User.query.filter_by(username=author_username).first()
    if not user:
        return redirect(url_for('home'))
    # restituisce la pagina home.html passando come argomenti il titolo della pagina e tutti i pos
...
def a_post(post_id):
    ...
    # restituisce la pagina home.html passando come argomenti il titolo della pagina e tutti i post
    return render_template("a_post.html", title=f"post num.{post_id} by post.author", posts=post)
